...
class MacMiniController:
    ...
    def handle_received(self, frame: str):
        ...
        # Only react to messages from ARMx -> MASTER
        if not (src and src.startswith("ARM") and dest == "MASTER"):
            if DEBUG_FRAMES:
                print(f"[RECEIVED] (ignored) src={src} dest={dest} verb={verb} cmd={cmd} payload={payload}")
            return

        try:
            arm_dev = DeviceType[src]
        except Exception:
            if DEBUG_FRAMES:
                print(f"[RECEIVED] (ignored unknown arm) {src}")
            return

        if verb == "CONFIRM" and cmd == RequestType.SEND_STAR.value:
            self._on_confirm_send_star(arm_dev)
        elif verb == "REQUEST" and cmd == RequestType.STAR_ARRIVED.value:
            self._on_star_arrived(arm_dev)
        else:
            # Silently ignore other confirms/events
            if DEBUG_FRAMES:
            ...
            while True:
                ...
                with self.lock:
                    for star in self.arms.values():
                        ...
                        elif star.state == StarState.DISPATCHING:
                            # (Re)try if first time or ACK timeout passed
                            due = (star.retry_count == 0) or (
                                star.last_send_attempt and (now - star.last_send_attempt) >= ACK_TIMEOUT
                            )
                            if due:
                                if star.retry_count < MAX_SEND_RETRIES:
                                    send_queue.append(star)
                                else:
                                    print(f"[ERROR] {star.arm.value} no CONFIRM after {MAX_SEND_RETRIES} SEND_STAR attempts. Resetting arm.")
                                    self._reset_arm(star)
    ...
    def _try_send_star(self, star: Star):
        ...
        star.retry_count += 1
        # No noisy print here beyond [SEND]; keep console clean
        # (If you like, uncomment:) print(f"[SEND_STAR] {star.arm.value} attempt {star.retry_count} (brightness={star.brightness})")
